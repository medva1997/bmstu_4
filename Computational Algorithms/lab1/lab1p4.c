...
float **interp_coef(float **points, float **tmp, float *c, int n)
{
  ...
  for (int i = 1; i < n+1; i++)
  {
    float a = points[i-1][1];
    float b = tmp[i][1];
    b -= tmp[i][0]*(c[i+1]+2*c[i])/3;
    float d = (c[i+1] - c[i])/(3*tmp[i][0]);
    t(Interp,i,a,b,c[i],d);
  }
  printf("INTERP\n");
...
float **interpolation(float **points,int n)
{
  ...
  if (n >= 2)
    ...
    //c[1] = (-slau[n][3] - slau[n][0] * progon[n][0])/(-slau[n][1] + slau[n][0]*progon[n][1]);
  else
    c[n] = 0;
    //c[1] = 0;
  int ii = n-1;
...
float getY(float x, float **points, float **interp, int n)
{
    ...
    float y = interp[i][0] + dx*(interp[i][1] + dx*(interp[i][2]+ dx*interp[i][3]));
    return y;
}
int main(void)
{
